  ...
  <Page auto-content-height>
    <BasicTable table-title="登录日志列表">
      <template #toolbar-tools>
        <Space>
          <Button v-access:code="['monitor:logininfor:remove']" @click="handleClear">
            {{ $t('pages.common.clear') }}
          </Button>
          <Button v-access:code="['monitor:logininfor:export']" @click="handleDownloadExcel">
            {{ $t('pages.common.export') }}
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary"
            v-access:code="['monitor:logininfor:remove']" @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </Button>
          <Button :disabled="!canUnlock" type="primary" v-access:code="['monitor:logininfor:unlock']"
            @click="handleUnlock">
      ...
      <template #action="{ row }">
        <Space>
          ...
          <Popconfirm :get-popup-container="getVxePopupContainer" placement="left" title="确认删除?"
            ...
            <ghost-button danger v-access:code="['monitor:logininfor:remove']" @click.stop="">
              {{ $t('pages.common.delete') }}
            </ghost-button>
          </Popconfirm>
        </Space>
