...
import { message, Modal, Popconfirm, Space, Button } from 'ant-design-vue';
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="代码生成列表">
      <template #toolbar-tools>
        <Space>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary" v-access:code="['tool:gen:remove']"
            @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" v-access:code="['tool:gen:code']" @click="handleBatchGen">
            {{ $t('pages.common.generate') }}
          </Button>
          <Button type="primary" v-access:code="['tool:gen:import']" @click="handleImport">
            {{ $t('pages.common.import') }}
          </Button>
        </Space>
      </template>
      <template #action="{ row }">
        <Button size="small" type="link" v-access:code="['tool:gen:preview']" @click.stop="handlePreview(row)">
          {{ $t('pages.common.preview') }}
        </Button>
        <Button size="small" type="link" v-access:code="['tool:gen:edit']" @click.stop="handleEdit(row)">
          {{ $t('pages.common.edit') }}
        </Button>
        <Popconfirm :get-popup-container="getVxePopupContainer" :title="`确认同步[${row.tableName}]?`" placement="left"
          @confirm="handleSync(row)">
          <Button size="small" type="link" v-access:code="['tool:gen:edit']" @click.stop="">
            {{ $t('pages.common.sync') }}
          </Button>
        </Popconfirm>
        <Button size="small" type="link" v-access:code="['tool:gen:code']" @click.stop="handleDownload(row)">
          生成代码
        </Button>
        <Popconfirm :get-popup-container="getVxePopupContainer" :title="`确认删除[${row.tableName}]?`" placement="left"
          @confirm="handleDelete(row)">
          <Button danger size="small" type="link" v-access:code="['tool:gen:remove']" @click.stop="">
            {{ $t('pages.common.delete') }}
          </Button>
        </Popconfirm>
      </template>
