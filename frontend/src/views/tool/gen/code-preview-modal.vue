...
import { Alert, Skeleton, Tree, Button } from 'ant-design-vue';
...
<template>
  <BasicModal :footer="false" :fullscreen="true" :fullscreen-button="false" :title="modalTitle">
    <div v-if="currentCodeData" class="flex gap-[8px]">
      <div class="h-[calc(100vh-80px)] w-[300px] overflow-y-scroll">
        <Tree v-if="treeData.length > 0" :show-line="{ showLeafIcon: false }" :tree-data="treeData" :virtual="false"
          default-expand-all @select="handleSelect">
        ...
        <Alert class="mt-2" show-icon message="👆显示的名称为模板的文件名，非最终下载文件名..." />
      </div>
      <CodeMirror v-model="codeContent" :language="language"
        class="h-[calc(100vh-80px)] w-full overflow-y-scroll text-[16px]" readonly />
      <div class="fixed right-20 top-20">
        <Button @click="copy(codeContent)">复制</Button>
      </div>
    </div>
