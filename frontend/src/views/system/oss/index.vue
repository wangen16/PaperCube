    ...
    <BasicTable table-title="文件列表">
      <template #toolbar-tools>
        <Space>
          ...
          <Button v-access:code="['system:ossConfig:list']" @click="handleToSettings">
            配置管理
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary" v-access:code="['system:oss:remove']"
            @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </Button>
          <Button v-access:code="['system:oss:upload']" @click="fileUploadApi.open">
            文件上传
          </Button>
          <Button v-access:code="['system:oss:upload']" @click="imageUploadApi.open">
            图片上传
          </Button>
        </Space>
      </template>
      <template #url="{ row }">
        ...
        <Image :key="row.ossId" v-if="preview && isImageFile(row.url)" :src="row.url" height="50px"
        ...
        <!-- pdf预览 使用浏览器开新窗口 -->
        <span v-else-if="preview && isPdfFile(row.url)"
          class="icon-[vscode-icons--file-type-pdf2] size-10 cursor-pointer" @click.stop="pdfPreview(row.url)"></span>
        <span v-else>{{ row.url }}</span>
      </template>
      <template #action="{ row }">
        <Space>
          <ghost-button v-access:code="['system:oss:download']" @click="handleDownload(row)">
            {{ $t('pages.common.download') }}
          </ghost-button>
          <Popconfirm :get-popup-container="getVxePopupContainer" placement="left" title="确认删除？"
            @confirm="handleDelete(row)">
            <ghost-button danger v-access:code="['system:oss:remove']" @click.stop="">
              {{ $t('pages.common.delete') }}
            </ghost-button>
          </Popconfirm>
        </Space>
