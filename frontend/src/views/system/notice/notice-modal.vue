...
import {
  contentWithOssIdTransform,
  normalizeRichTextContent,
} from '#/components/tinymce/src/helper';
...
function createDefaultValues(): FormData {
  return {
    ...defaultValues,
  };
}

const formData = reactive<FormData>(createDefaultValues());
...
const [BasicModal, modalApi] = useVbenModal({
  ...
  onOpenChange: async (isOpen) => {
    ...
    if (isUpdate.value && id) {
      ...
      Object.assign(formData, createDefaultValues(), filterRecord, {
        noticeContent: normalizeRichTextContent(filterRecord.noticeContent),
      });
    } else {
      Object.assign(formData, createDefaultValues());
    }
