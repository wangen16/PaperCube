...
type MenuFormValues = Record<string, any>;
...
const [BasicDrawer, drawerApi] = useVbenDrawer({
  ...
  async onOpenChange(isOpen) {
    ...
    if (id) {
      ...
      if (record) {
        await formApi.setValues(normalizeMenuFormValues(record));
      }
    } else {
      // 加载菜单树选择
...
async function handleConfirm() {
  try {
    ...
    const data = cloneDeep(normalizeMenuFormValues(await formApi.getValues()));
...
function normalizeMenuFormValues(values: MenuFormValues) {
  return {
    ...values,
    isCache: values.isCache != null ? String(values.isCache) : '0',
    isFrame: values.isFrame != null ? String(values.isFrame) : '1',
    query: values.query ?? values.queryParam ?? '',
    status: values.status != null ? String(values.status) : '0',
    visible: values.visible != null ? String(values.visible) : '0',
  };
}
</script>
