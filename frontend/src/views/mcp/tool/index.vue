  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="MCP工具列表">
      <template #toolbar-tools>
        <Space>
          <Button v-access:code="['mcp:tool:export']" @click="handleDownloadExcel">
            {{ $t('pages.common.export') }}
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary" v-access:code="['mcp:tool:remove']"
            @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </Button>
          <Button type="primary" v-access:code="['mcp:tool:add']" @click="handleAdd">
            {{ $t('pages.common.add') }}
          </Button>
        </Space>
      </template>
      <template #status="{ row }">
        <TableSwitch v-model:value="row.status" :api="() => mcpToolChangeStatus(row)"
          :disabled="!hasAccessByCodes(['mcp:tool:edit'])" :checked-value="'ENABLED'" :unchecked-value="'DISABLED'"
          @reload="tableApi.query()" />
      </template>
      <template #action="{ row }">
        <Space>
          <ghost-button v-access:code="['mcp:tool:query']" @click.stop="handleTest(row)">
            测试
          </ghost-button>
          <ghost-button v-access:code="['mcp:tool:edit']" @click.stop="handleEdit(row)">
            {{ $t('pages.common.edit') }}
          </ghost-button>
          <Popconfirm :get-popup-container="getVxePopupContainer" placement="left" title="确认删除？"
            @confirm="handleDelete(row)">
            <ghost-button danger v-access:code="['mcp:tool:remove']" @click.stop="">
              {{ $t('pages.common.delete') }}
            </ghost-button>
          </Popconfirm>
        </Space>
