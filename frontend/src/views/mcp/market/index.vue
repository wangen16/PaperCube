...
import { Modal, Popconfirm, Space, message, Button } from 'ant-design-vue';
  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="MCP市场列表">
      <template #toolbar-tools>
        <Space>
          <Button v-access:code="['mcp:market:export']" @click="handleDownloadExcel">
            {{ $t('pages.common.export') }}
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary" v-access:code="['mcp:market:remove']"
            @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </Button>
          <Button type="primary" v-access:code="['mcp:market:add']" @click="handleAdd">
            {{ $t('pages.common.add') }}
          </Button>
        </Space>
      </template>
      <template #status="{ row }">
        <TableSwitch v-model:value="row.status" :api="() => mcpMarketChangeStatus(row)"
          :disabled="!hasAccessByCodes(['mcp:market:edit'])" :checked-value="'ENABLED'" :unchecked-value="'DISABLED'"
          @reload="tableApi.query()" />
      </template>
      <template #action="{ row }">
        <Space>
          <ghost-button v-access:code="['mcp:market:edit']" @click.stop="handleRefresh(row)">
            刷新
          </ghost-button>
          <ghost-button v-access:code="['mcp:market:edit']" @click.stop="handleEdit(row)">
            {{ $t('pages.common.edit') }}
          </ghost-button>
          <Popconfirm :get-popup-container="getVxePopupContainer" placement="left" title="确认删除？"
            @confirm="handleDelete(row)">
            <ghost-button danger v-access:code="['mcp:market:remove']" @click.stop="">
              {{ $t('pages.common.delete') }}
            </ghost-button>
          </Popconfirm>
        </Space>
