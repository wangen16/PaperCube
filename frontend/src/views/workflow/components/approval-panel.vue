...
import { Card, Divider, message, TabPane, Tabs, Button } from 'ant-design-vue';
...
<template>
  <div :class="cn('thin-scrollbar', 'flex flex-1 overflow-y-hidden')">
    <Card v-if="task" :body-style="{ overflowY: 'auto', height: '100%' }" :loading="loading"
      class="thin-scrollbar flex-1 overflow-y-hidden" size="small">
      <template #title>
      ...
      <div class="flex flex-col gap-5 p-4">
        <div class="flex flex-col gap-3">
          <div class="flex items-center gap-2">
            ...
            <div>
              <component :is="renderDict(task.flowStatus, DictEnum.WF_BUSINESS_STATUS)" />
            </div>
          </div>

          <div class="flex items-center gap-2">
            <VbenAvatar :alt="task?.createByName ?? ''" class="bg-primary size-[28px] rounded-full text-white" src="" />
        ...
        <Tabs v-if="currentFlowInfo" class="flex-1">
          <TabPane key="1" tab="审批详情">
            <ApprovalDetails :current-flow-info="currentFlowInfo" :task="task" />
          </TabPane>
      ...
      <FlowActions v-if="showFooter" :type="type" :task="task" :button-permissions="buttonPermissions"
        @reload="$emit('reload')" />
    </Card>
