...
import { Dropdown, Menu, MenuItem, Modal, Space, Button } from 'ant-design-vue';
  ...
  <div :class="cn(
    ...
    <div class="flex justify-end">
      <Space v-if="type === 'myself'">
        <Button v-if="revocable" danger ghost type="primary" :icon="h(RollbackOutlined)" @click="handleCancel">
          撤销申请
        </Button>
        <Button type="primary" ghost v-if="editableAndRemoveable" :icon="h(EditOutlined)" @click="handleEdit">
          重新编辑
        </Button>
        <Button v-if="editableAndRemoveable" danger ghost type="primary" :icon="h(EditOutlined)" @click="handleRemove">
          删除
        </Button>
      </Space>
      <Space v-if="type === 'approve'">
        <Button type="primary" ghost :icon="h(CheckOutlined)" @click="handleApproval">
          通过
        </Button>
        <Button v-if="buttonPermissions?.termination" danger ghost type="primary" :icon="h(ExclamationCircleOutlined)"
          @click="handleTermination">
          终止
        </Button>
        <Button v-if="buttonPermissions?.back" danger ghost type="primary" :icon="h(ArrowLeftOutlined)"
          @click="handleRejection">
          驳回
        </Button>
        <Dropdown :get-popup-container="getPopupContainer" placement="bottomRight">
          <template #overlay>
            <Menu>
              <MenuItem v-if="buttonPermissions?.trust" key="1" @click="() => delegationModalApi.open()">
              <UserOutlined class="mr-2" />委托
              </MenuItem>
              <MenuItem v-if="buttonPermissions?.transfer" key="2" @click="() => transferModalApi.open()">
              <RollbackOutlined class="mr-2" /> 转办
              </MenuItem>
              <MenuItem v-if="showMultiActions && buttonPermissions?.addSign" key="3"
                @click="() => addSignatureModalApi.open()">
              <UsergroupAddOutlined class="mr-2" /> 加签
              </MenuItem>
              <MenuItem v-if="showMultiActions && buttonPermissions?.subSign" key="4"
                @click="() => reductionSignatureModalApi.open()">
              <UsergroupDeleteOutlined class="mr-2" /> 减签
              </MenuItem>
            </Menu>
          </template>
          <Button v-if="showButtonOther" :icon="h(MenuOutlined)">
            其他
          </Button>
        </Dropdown>
        ...
        <ReductionSignatureModal mode="multiple" @finish="handleReductionSignature" />
      </Space>
