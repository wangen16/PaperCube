...
<template>
  <BasicModal>
    <div class="flex min-h-[600px]">
      <DeptTree v-model:select-dept-id="selectDeptId" :show-search="false" class="w-[230px]"
        @reload="() => tableApi.reload()" @select="handleDeptQuery" />
      <div class="h-[600px] w-[450px]">
        <BasicTable>
          <template #user="{ row }">
            <div class="flex items-center gap-2">
              <VbenAvatar :alt="row.nickName" :src="row.avatar ?? ''" :class="{ 'bg-primary': !row.avatar }"
      ...
      <div class="flex h-[600px] flex-col">
        ...
        <RightBasicTable id="user-select-right-table">
          ...
          <template #action="{ row }">
            <Button size="small" @click="handleRemoveItem(row)">
              移除
            </Button>
          </template>
        </RightBasicTable>
