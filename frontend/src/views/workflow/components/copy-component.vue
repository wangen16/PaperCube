...
import { Avatar, AvatarGroup, Tooltip, Button } from 'ant-design-vue';
...
<template>
  <div class="flex items-center gap-2">
    <AvatarGroup v-if="userListModel.length > 0">
      <Tooltip v-for="user in displayedList" :key="user.userId" :title="user.nickName" placement="top">
        <div>
          <VbenAvatar :alt="user?.nickName ?? ''"
            class="bg-primary size-[36px] cursor-pointer rounded-full border text-white" src="" />
        </div>
      </Tooltip>
      <Tooltip :title="`等${userListModel.length - props.ellipseNumber}人`" placement="top">
        <Avatar v-if="userListModel.length > ellipseNumber"
    ...
    <UserSelectModal :allow-user-ids="allowUserIds" @cancel="$emit('cancel')" @finish="handleFinish" />
  </div>
</template>
