...
import { Image, Modal, Popconfirm, Space, Spin, Switch, Tooltip, Button } from 'ant-design-vue';
    ...
    <BasicTable table-title="厂商管理列表">
      <template #toolbar-tools>
        <Space>
          ...
          <Button v-access:code="['system:provider:export']" @click="handleDownloadExcel">
            {{ $t('pages.common.export') }}
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary"
            v-access:code="['system:provider:remove']" @click="handleMultiDelete">
            {{ $t('pages.common.delete') }}
          </Button>
          <Button type="primary" v-access:code="['system:provider:add']" @click="handleAdd">
            {{ $t('pages.common.add') }}
          </Button>
        </Space>
      </template>
      <template #providerIcon="{ row }">
        <Image v-if="preview && isImageFile(row.providerIcon)" :key="row.id" :src="row.providerIcon" height="35px"
          width="35px" style="object-fit: cover" preview>
      ...
      <template #action="{ row }">
        <Space>
          <ghost-button v-access:code="['system:provider:edit']" @click.stop="handleEdit(row)">
            {{ $t('pages.common.edit') }}
          </ghost-button>
          <Popconfirm :get-popup-container="getVxePopupContainer" placement="left" title="确认删除？"
            @confirm="handleDelete(row)">
            <ghost-button danger v-access:code="['system:provider:remove']" @click.stop="">
              {{ $t('pages.common.delete') }}
            </ghost-button>
          </Popconfirm>
        </Space>
