  ...
  <Page :auto-content-height="true">
    <BasicTable table-title="聊天消息列表">
      <template #toolbar-tools>
        <Space>
          <Button v-access:code="['system:message:export']" @click="handleDownloadExcel">
            {{ $t('pages.common.export') }}
          </Button>
          <Button :disabled="!vxeCheckboxChecked(tableApi)" danger type="primary"
            v-access:code="['system:message:remove']" @click="handleMultiDelete">
      ...
      <template #action="{ row }">
        <Space>
          <Popconfirm :get-popup-container="getVxePopupContainer" placement="left" title="确认删除？"
            ...
            <ghost-button danger v-access:code="['system:message:remove']" @click.stop="">
              {{ $t('pages.common.delete') }}
            </ghost-button>
          </Popconfirm>
        </Space>
