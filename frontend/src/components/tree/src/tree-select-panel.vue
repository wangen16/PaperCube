...
import { Checkbox, Tree, Button } from 'ant-design-vue';
...
<template>
  <div class="bg-background w-full rounded-lg border-[1px] p-[12px]">
    ...
    <div class="flex flex-wrap items-center justify-between border-b-[1px] py-2">
      <Button size="small" @click="handleExpandOrCollapseAll">
        展开/折叠全部
      </Button>
      <Checkbox v-model:checked="selectAllStatus" @change="handleCheckedAllChange">
        全选/取消全选
      </Checkbox>
      <Checkbox v-model:checked="checkStrictly" @change="handleCheckStrictlyChange">
        父子节点关联
      </Checkbox>
    </div>
    <div class="py-2">
      <Tree :check-strictly="!checkStrictly" v-model:checked-keys="computedCheckedKeys"
        v-model:expanded-keys="expandedKeys" :checkable="true" :field-names="fieldNames" :selectable="false"
        :tree-data="treeData">
