...
import { Image, ImagePreviewGroup, Upload, Button } from 'ant-design-vue';
...
<template>
  <div>
    <Upload v-model:file-list="innerFileList" :class="{ 'upload-animation__disabled': !withAnimation }"
      :list-type="listType" :accept="accept" :disabled="disabled" :directory="directory" :max-count="maxCount"
      :progress="{ showInfo: true }" :multiple="multiple" :before-upload="beforeUpload" :custom-request="customRequest"
      @preview="currentPreview" @change="handleChange" @remove="handleRemove">
      <div v-if="innerFileList?.length < maxCount && listType === 'picture-card'">
        <PlusOutlined />
        <div class="mt-[8px]">{{ $t('component.upload.upload') }}</div>
      </div>
      <Button v-if="innerFileList?.length < maxCount && listType !== 'picture-card'" :disabled="disabled">
        <UploadOutlined />
    ...
    <slot name="helpMessage" v-bind="{ maxCount, disabled, maxSize, accept }">
      <I18nT v-if="helpMessage" scope="global" keypath="component.upload.uploadHelpMessage" tag="div"
        class="text-[14px] leading-[1.5] text-black/45 dark:text-white/45" :class="{
