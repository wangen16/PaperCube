...
function backMenuToVbenMenu(
  ...
  menuList.forEach((menu) => {
    ...
    // 处理 alwaysShow = false 的场景：如果只有一个子菜单，应将其提升隐藏父级目录
    if (menu.alwaysShow === false && menu.children && menu.children.length === 1) {
      const child = menu.children[0];
      if (child && vbenRoute.meta) {
        vbenRoute.meta.hideChildrenInMenu = true;
        // 把子元素的展示信息提上来，模拟真正的提升展示
        vbenRoute.meta.title = child.meta?.title || vbenRoute.meta.title;
        vbenRoute.meta.icon = child.meta?.icon || vbenRoute.meta.icon;
        vbenRoute.name = child.name || vbenRoute.name;

        // 拼接正确的重定向路径
        let childPathPart = child.path.startsWith('/') ? child.path : `/${child.path}`;
        let fullPath = `${vbenRoute.path}${childPathPart}`.replace(/\/+/g, '/');
        if (/^https?:\/\//.test(child.path)) {
          fullPath = child.path;
        }
        vbenRoute.redirect = fullPath;
      }
    }
